...
server <- function(input, output) {
  ...
  output_topdeals <- reactive({
    ozb_data %>% head(30*input$pages) %>% group_by(tags) %>% slice(which.max(upvote))
  })
  
  output$top_deals <- renderText({
    if(is.null(input$hover$x)) return()
    else {
      hover_tag <- output_tags()$.[round(input$hover$x)]
      output_topdeals()$title[output_topdeals()$tags == hover_tag]
    }
  })
